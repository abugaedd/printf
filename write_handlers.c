...
/**
 * write_num - will wirte a number using a buffer
		...
		char padd, char extra_c)
{
	...
	if (prec == 0 && ind == BUFF_SIZE - 2 && buffer[ind] == '0' && width == 0)
		return (0); /* printf(".0d", 0) No char will print */
	...
	if (width > length)
	{
		for (i = 1; i < width - length + 1; i++)
			buffer[i] = padd;
		buffer[i] = '\0';
		if (flags & F_MINUS && padd == ' ') /*Assing extra char left of buff */
		{
			if (extra_c)
				buffer[--ind] = extra_c;
...
int write_unsgnd(int is_negative, int ind, char buffer[],
		int flags, int width, int precision, int size)
{
	/*The num is stored at buffer's right and starts at i */
